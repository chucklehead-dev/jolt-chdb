...
scratch=$(mktemp -d /tmp/jolt-chdb-stream-insert-threadstatus.XXXXXX)
...
if ! script -qefc \
  "$jolt_command -A:test -m jdbc.chdb-stream-insert-threadstatus-probe" \
...
if ! grep -Fq 'PASS: streaming-insert ThreadStatus probe' "$transcript"; then
  sed -n '1,240p' "$transcript" >&2
  exit 1
fi

printf 'PASS: no ThreadStatus diagnostic in streaming-insert lifecycle\n'
