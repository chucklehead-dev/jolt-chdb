...
(defn- fake-open-operations [calls clocks close-count cleanup-count]
  {:classification-sql! (fn [sql _] sql)
   ...
   :create-checkpoint! (fn [_ database scratch]
                         (let [path (.resolve ^Path scratch "new-checkpoint.tar.gz")]
...
(defn- run-deterministic-checks! []
  ...
  (let [store (prepared-wal-store)
        ...
        opened (durable/open-writer!
                {:store store :owner "new-writer" :instance "new-instance"
                 ...
                 :operations (fake-open-operations calls clocks close-count
                                                   cleanup-count)})]
  ...
  (let [store (prepared-checkpoint-store)
        ...
        close-count (atom 0)
        cleanup-count (atom 0)
        operations (assoc (fake-open-operations calls clocks close-count cleanup-count)
                          :restore-database!
                          (fn [_ _ _]
  ...
  (let [namespace (backend/memory-backend)
        ...
        cleanup-count (atom 0)
        operations
        (assoc (fake-open-operations calls clocks close-count cleanup-count)
               :query-native!
               (fn [_ _ params]
                 {:labels ["value"] :rows [[(first params)]] :count 1})
               :execute-native!
  ...
  (let [store (backend/memory-backend)
    ...
    (check "read-only open reports not-found"
           ...
           (error-type
            #(durable/open-reader!
              {:store store
               :operations (fake-open-operations
  ...
  (let [store (prepared-wal-store)
        ...
        close-count (atom 0)
        cleanup-count (atom 0)
        base-operations (fake-open-operations
                         calls clocks close-count cleanup-count)
