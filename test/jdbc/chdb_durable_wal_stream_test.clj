  ...
  (let [next-lf-var (ns-resolve 'jdbc.chdb.durable 'next-lf-index)
    ...
    (check "typed LF finder has the emitted fast-path contract"
           ...
           (and (str/includes? scan-source
                ...
                (str/includes? scan-source "(byte 10)")
                (str/includes? scan-source "(unchecked-inc index)")
                (not (str/includes? scan-source "bit-and"))))
    (check "WAL visitation uses only the typed LF finder for byte scanning"
           true
           (and (str/includes? visit-source
                               "(next-lf-index chunk start read-count)")
