...
- `execute-and-flush!`: execute one fully materialized mutation and publish its
  recovery state as one FIFO worker request, returning only that caller's
  confirmed or reconciled publication receipt. This is the safe composition
  boundary for a shared writer; it is not equivalent to separately calling
  `execute!` and `flush!`;
